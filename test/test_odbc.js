...
async function performAction(client, key, action) {
    ...
    if(!report["odbc"]){
        report["odbc"] = {"manual": {}}
    }else if(!report["odbc"]["manual"]){
        report["odbc"]["manual"] = {}
    }

    report["odbc"]["manual"][key] = d

}
// Mocha test suite for performance testing
describe('ODBC Performance Tests', function() {
    this.timeout(500000); // Set Mocha timeout to 30 seconds for long-running tests
    ...
    it('should perform a select operation quickly', async function() {
        ...
        report["odbc"]["manual"]["select"] = duration
    });
